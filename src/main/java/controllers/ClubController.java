...
public class ClubController implements Initializable {
    ...
    public void exportToCSV() {
        System.out.println("Exportation des clubs en cours...");

        String fileName = "Clubs.csv";

        try (PrintWriter writer = new PrintWriter(new FileWriter(fileName))) {

            // En-têtes
            writer.println("ID,Nom,Type,Date de Création,Utilisateur,Logo");
            ...
            while (rs.next()) {
                ...
                writer.printf("%d,%s,%s,%s,%s,%s%n", id, nom, type, dateCreation, userId, logo);
            }

            System.out.println("Fichier CSV généré avec succès !");

        } catch (IOException | SQLException e) {
            System.err.println("Erreur lors de l'exportation des clubs :");
            e.printStackTrace();
        }
    }
        ...
        clubListView.setCellFactory(param -> new ListCell<Club>() {
            ...
            protected void updateItem(Club club, boolean empty) {
                ...
                if (empty || club == null) {
                    ...
                    if (club.getQrCodeBytes() != null && club.getQrCodeBytes().length > 0) {
                        try {
                            // Affichage du QR code
                            ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(club.getQrCodeBytes());
                            Image qrImage = new Image(byteArrayInputStream);

                            // Vérifie si l'image est valide
                            if (qrImage.isError()) {
                                System.out.println("Erreur lors de la création du QR Code : " + qrImage.getException().getMessage());
                                setGraphic(null);
                            } else {
                    ...
                    } else if (club.getLogo() != null && !club.getLogo().isEmpty()) {
                        // Afficher le logo si le QR code n'est pas disponible
                        try {
    ...
    private void refreshListView() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Récupérer le QR Code en bytes et le stocker dans le modèle
                byte[] qrBytes = rs.getBytes("qr_code");
                if (qrBytes != null) {
                    club.setQrCodeBytes(qrBytes); // Stocke les bytes du QR code
                }
    ...
    private void refreshTable() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                byte[] qrBytes = rs.getBytes("qr_code");
                if (qrBytes != null) {
                    club.setQrCodeBytes(qrBytes); // stocker brut dans le modèle
                }
                clubs.add(club);
            }
            clubListView.setItems(clubs);
    ...
    private byte[] generateQRCodeBytes(String content) throws Exception {
        ...
        return baos.toByteArray(); // ✅ retourne les bytes à stocker dans la BDD
    }
    @FXML
    private void createClub() {
        ...
        try {
            String qrContent = "Nom: " + nom + ", Type: " + type + ", Date: " + dateCreation;
            byte[] qrCodeBytes = generateQRCodeBytes(qrContent); // ✅ on récupère les bytes

            PreparedStatement ps = connection.prepareStatement(
                    "INSERT INTO club(nom, type, logo, date_creation, user_id, qr_code) VALUES (?, ?, ?, ?, ?, ?)"
            );
            ps.setString(1, nom);
            ps.setString(2, type);
            ps.setString(3, logoPath);
            ps.setDate(4, Date.valueOf(dateCreation));
            ps.setInt(5, 1); // Remplacer par l'ID réel de l'utilisateur connecté
            ps.setBytes(6, qrCodeBytes); // ✅ stocke l'image QR en base

            ps.executeUpdate();
            refreshTable();
            clear();
            showNotification("Club ajouté avec succès avec QR Code !");
        } catch (SQLException e) {
            e.printStackTrace();
            showNotification("Erreur SQL : " + e.getMessage());
        } catch (Exception e) {
            e.printStackTrace();
            showNotification("Erreur QR : " + e.getMessage());
        }
    }
