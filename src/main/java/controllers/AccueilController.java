...
public class AccueilController implements Initializable {
    ...
    private void navigateToProfile(ActionEvent event) {
        // Fetch the currently active user details
        String username = String.valueOf(SessionManager.getCurrentUser());
        if (username == null) {
            showAlert("Profil", "Aucun utilisateur actif trouvé.", Alert.AlertType.WARNING);
            return;
        }

        // Fetch the last login time dynamically
        String lastLoginTime = SessionManager.getLastLoginTime();

        // Show user details in an alert dialog
        String userDetails = "Nom d'utilisateur : " + username + "\n"
                + "Rôle : User\n"
                + "Dernière connexion : " + lastLoginTime;
        showAlert("Détails du Profil", userDetails, Alert.AlertType.INFORMATION);
    }
*/
    @FXML
