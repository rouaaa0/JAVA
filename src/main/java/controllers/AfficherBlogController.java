...
import models.Blog;
import service.ReadingTimeService;
...
// Add these imports at the top of your AfficherBlogController.java file
...
import service.TextToSpeechService;
import service.ReadingTimeService;
import service.ReadingTimeService.ReadingTimeResult;
...
public class AfficherBlogController {
   ...
   /* @FXML
    ...
    @FXML
    private TextArea frenchOutput;

    private HttpClient httpClient;
    private ScheduledExecutorService executorService;
    private String lastTranslationRequest = "";
    @FXML
    private TextArea languageDetectionInput;

    @FXML
    private TextField languageDetectionOutput;

    @FXML
    private Button detectLanguageButton;
    // Add these field declarations at the top of your AfficherBlogController class
    @FXML
    ...
    public void initialize() {
        // Initialize HTTP client and executor service for translation
        httpClient = HttpClient.newHttpClient();
        readingTimeService = new ReadingTimeService();
        executorService = Executors.newSingleThreadScheduledExecutor();
        detectLanguageButton.setOnAction(event -> handleDetectLanguage());
        ttsService = new TextToSpeechService();

        // Make sure playback controls are hidden initially
        if (playbackControls != null) {
            playbackControls.setVisible(false);
        }
        // Set up text change listener with delay for translation
        englishInput.textProperty().addListener((obs, oldText, newText) -> {
            if (!newText.trim().isEmpty() && !newText.trim().equals(lastTranslationRequest)) {
                ...
                executorService.schedule(() -> {
                    lastTranslationRequest = newText.trim();
                    translateText(newText.trim());
   ...
   /* private void refreshCards() {
    ...
    private VBox createBlogCard(Blog blog) {
        ...
        Label readingTimeLabel = new Label("Calculating reading time...");
        readingTimeLabel.setStyle("-fx-font-style: italic; -fx-text-fill: #555555;");
        ...
        card.getChildren().addAll(titleText, descriptionText, createdAtText, updatedAtText, readingTimeLabel);

        // Estimate reading time after adding the label
        readingTimeService.estimateReadingTime(
                blog.getDescription(),
                result -> Platform.runLater(() -> readingTimeLabel.setText(result.getFormattedReadingTime())),
                error -> Platform.runLater(() -> readingTimeLabel.setText("Reading time unavailable"))
        );
    ...
    private void handleAddAction(ActionEvent event) {
        ...
        try {
            ...
            if (generatedKeys.next()) {
                ...
                blogHistoryLogs.add("Blog updated: '" + selectedBlog.getTitle() + "'");
            }
  ...
  /*  @FXML
    ...
    private void translateText(String text) {
        try {
            ...
            String apiUrl = "https://lingva.ml/api/v1/en/fr/" + encodedText;
            ...
            httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                    ...
                    .thenAccept(jsonResponse -> {
                        String translation = parseTranslationFromJson(jsonResponse);
                        javafx.application.Platform.runLater(() -> {
                            frenchOutput.setText(translation);
                        });
                    })
                    .exceptionally(e -> {
                        javafx.application.Platform.runLater(() -> {
                            frenchOutput.setText("Translation error");
                        });
                        return null;
                    });
        } catch (Exception e) {
            javafx.application.Platform.runLater(() -> {
                frenchOutput.setText("Error: " + e.getMessage());
            });
        }
    }

    private String parseTranslationFromJson(String json) {
        try {
    ...
    @FXML
    private void handleDetectLanguage() {
        String text = descriptionField.getText().trim();

        if (text.isEmpty()) {
            // Show an error if no text was entered
            // Replace this with your actual alert/error display method
            System.out.println("Error: Please enter some text");
            return;
        }

        String detectedLanguage = detectLanguage(text);
        languageDetectionOutput.setText("language: " + detectedLanguage);
    }

    /**
     * Calls the API-Ninjas text language detection API.
     */
    private String detectLanguage(String text) {
        ...
        if (lowerText.contains("bonjour") || lowerText.contains("salut") ||
                lowerText.contains("merci") || lowerText.contains("oui") ||
                lowerText.contains("non")) {
            return "French";
        }

        // Check for English keywords
        else if (lowerText.contains("hello") || lowerText.contains("hi") ||
                lowerText.contains("thanks") || lowerText.contains("yes") ||
                lowerText.contains("no")) {
            return "English";
        }

        // Check for Arabic keywords and characters
        else if (lowerText.contains("مرحبا") || lowerText.contains("سلام") ||
                lowerText.contains("شكرا") || containsArabicCharacters(lowerText)) {
            return "Arabic";
        }
    ...
    /**
     * Parses the JSON response from the language detection API.
     */
    private String parseLanguageResponse(String jsonResponse) {
        try {
            // Simple JSON parsing (you could use a proper JSON library like Jackson if available)
            int languageNameStart = jsonResponse.indexOf("\"language\":\"") + 12;
            int languageNameEnd = jsonResponse.indexOf("\"", languageNameStart);
            String languageName = jsonResponse.substring(languageNameStart, languageNameEnd);

            int codeStart = jsonResponse.indexOf("\"code\":\"") + 8;
            int codeEnd = jsonResponse.indexOf("\"", codeStart);
            String code = jsonResponse.substring(codeStart, codeEnd);
    ...
    private void handleReadAloud() {
        if (selectedBlog == null) {
            showAlert("Error", "No blog selected", "Please select a blog to read aloud.");
            return;
        }

        // Get the blog description to read
        String textToRead = selectedBlog.getDescription();

        // Show loading indicator (optional)
        readAloudButton.setDisable(true);
        progressIndicator.setVisible(true);

        // Call the TTS service
        ttsService.convertTextToSpeech(
                textToRead,
                "nova", // Default voice - you could add a voice selector if needed
                () -> {
                    // On success
                    Platform.runLater(() -> {
                        readAloudButton.setDisable(false);
                        progressIndicator.setVisible(false);
                        // Show playing indicator or controls
                ...
                (error) -> {
                    ...
                    Platform.runLater(() -> {
                        readAloudButton.setDisable(false);
                        progressIndicator.setVisible(false);
                        showAlert("Error", "Text-to-Speech Error",
                                "Failed to convert text to speech: " + error.getMessage());
                    });
                }
        );
    }

    // 4. Add a method to stop playback
    @FXML
    ...
    // 5. Clean up resources when controller is closed
    public void cleanup() {
        if (ttsService != null) {
            ttsService.shutdown();
        }
    ...
    private void estimateReadingTime(Blog blog) {
        if (blog == null || blog.getDescription() == null) {
            return;
        }

        // Show loading indicator or status message
        // For example: readingTimeLabel.setText("Calculating...");

        readingTimeService.estimateReadingTime(
                blog.getDescription(),
                result -> {
                    // On success, update UI with reading time
                    Platform.runLater(() -> {
                        String readingTimeText = result.getFormattedReadingTime();
                        // Update UI with reading time - could be a label in the blog card
                        // For example: blog.setReadingTime(readingTimeText);
                ...
                error -> {
                    ...
                    Platform.runLater(() -> {
                        // Show error message
                        // For example: readingTimeLabel.setText("Error estimating reading time");
                        System.err.println("Error estimating reading time: " + error.getMessage());
                    });
                }
        );
    }
    @FXML
    private Label readingTimeLabel; // Add this to your FXML

    private void updateReadingTimeForSelectedBlog() {
        if (selectedBlog != null && descriptionField.getText() != null) {
            readingTimeLabel.setText("Calculating...");
    ...
    public void goToBlogPage(ActionEvent event) {
        try {
            ...
            e.printStackTrace();
            showAlert("Error", "Failed to navigate to Blog page: " + e.getMessage(), String.valueOf(Alert.AlertType.ERROR));
        }
    }
    ...
    public void goToPostPage(ActionEvent event) {
        try {
            ...
            e.printStackTrace();
            showAlert("Error", "Failed to navigate to Post page: " + e.getMessage(), String.valueOf(Alert.AlertType.ERROR));
        }
    }
    ...
    public void goToClubPage(ActionEvent event) {
        try {
            ...
            e.printStackTrace();
            showAlert("Error", "Failed to navigate to Club page: " + e.getMessage(), String.valueOf(Alert.AlertType.ERROR));
        }
    }
    ...
    public void goToEventPage(ActionEvent event) {
        try {
            ...
            e.printStackTrace();
            showAlert("Error", "Failed to navigate to Event page: " + e.getMessage(), String.valueOf(Alert.AlertType.ERROR));
        }
    }
}
